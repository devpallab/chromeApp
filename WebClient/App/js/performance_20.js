...
PerformanceView.currentWeekNo = null;
...
PerformanceView.init = function (model) {
	...
	oSelf.leftPanelData = {};
    oSelf.model = model;
	oSelf.currentWeekNo = objCurrentWeekJsonData.Content.WeekNo;
	//console.log(oSelf.viewMode);
	...
	//console.log(oSelf.model.unitDetails.curUnitNumber);										
	...
	//console.log(oSelf.model.unitDetails.curWeekNumber);
	...
	oSelf.performanceTabHandler = new (function () {
				...
				if (objSkillBasedReportDataByWeekRange == 0) {
					...
					try {
						if (objSkillBasedReportDataByWeekRange.Status == "200" || objSkillBasedReportDataByWeekRange.Status == 200) {
							...
							$this.importGetClassSettings(fCallBack);
						}
						else {
		...
		// import data using GetClassSettings web service
		this.importGetClassSettings = function (fCallBack) {
			if (typeof fCallBack != 'function') {
				fCallBack = jQuery.noop;
			}
			var $this = this;
			objSettingsData = 0;
			GetClassSettings();
			$this.checkGetClassSettings(fCallBack);
		};
		// checking response of GetClassSettings web service
		this.checkGetClassSettings = function (fCallBack) {
			var $this = this;
			setTimeout(function () {
				if (objSettingsData == 0) {
					...
					try {
						if (objSettingsData.Status == "200" || objSettingsData.Status == 200) {
							//oSelf.model.skillBasedReportDataByWeekRange = objSkillBasedReportDataByWeekRange.Content;
							$this.importGetCurrentWeekForClass(fCallBack);
						}
						else {
							throw(objSettingsData.Error);
						}
					}
					catch (err) {
						if (err.ErrorCode != "U1065") {
		...
		// import data using GetClassSettings web service
		this.importGetCurrentWeekForClass = function (fCallBack) {
			if (typeof fCallBack != 'function') {
				fCallBack = jQuery.noop;
			}
			var $this = this;
			objCurrentWeekJsonData = 0;
			GetCurrentWeekForClass();
			$this.checkGetCurrentWeekForClass(fCallBack);
		};
		// checking response of GetCurrentWeekForClass web service
		this.checkGetCurrentWeekForClass = function (fCallBack) {
			var $this = this;
			setTimeout(function () {
				if (objCurrentWeekJsonData == 0) {
					$this.checkGetCurrentWeekForClass(fCallBack);
				}
				else {
					try {
						if (objCurrentWeekJsonData.Status == "200" || objCurrentWeekJsonData.Status == 200) {
							//oSelf.model.skillBasedReportDataByWeekRange = objSkillBasedReportDataByWeekRange.Content;
							//console.log(objCurrentWeekJsonData);
							$this.importGetClassUserLevel(fCallBack);
						}
						else {
	...
	oSelf.lineGraphAndDetailViewHandler = new (function () {
				...
				for (var iIdx = 0; iIdx < oSelf.model.gradeBookData.length; iIdx++) {
					...
					if (aAssignmentData.length > 0) {
						...
						/* if (oAssignmentData['ItemSubType'] == 'wrc') { */
						   /* following code only for iLit90 and iLit45 products */
						   /* if (oAssignmentData['ItemCategory'] == 'wrc') {	ILIT-1150					
					...
					$.each(oProgressDataWordCount, function (unitKey, wordVal) {
						var aInfo = unitKey.split("."),
							...
							oData = {
								...
								'ItemType':					'word_count',
								'WordCount':				(typeof wordVal == "string") ?  parseInt(wordVal.split("|")[0]) : wordVal
							};
						// Subject Search Key
						switch (aInfo[2]) {
							case 'RATA':
		...
		this.prepareFilters = function () {
			...
			oSelf.xAxistitle = PERFORMANCE.c_s_LABEL_WEEKS;
			
			// Unit & Week Filters 
				// IPP-4027 
			var aSliderEnds = $("#" + PERFORMANCE.c_s_SLIDER).slider("values"),
				iSliderMin = parseInt(aSliderEnds.first()),
				iSliderMax = parseInt(aSliderEnds.last()), // changes for slider manipulation
		...
		this.getFilteredData = function () { // Method 4
			...
			for (var sFilter in aFilters) {
				if (sFilter == 'WeekNumber') {
					...
					var aFilterParams = [ 'ItemCategory', 'ItemSubject' ],
						aFilteredData = [],
						aFData = [],
						aTempFilter = [];
	...
	$("#" + PERFORMANCE.c_s_CONTAINER_MAIN_DROPDOWN).empty().html(
		_.template(
			$("#" + PERFORMANCE.c_s_TEMPLATE_LINE_GRAPH_TYPE_DROPDOWN).html(), {
				...
				'classSettingData': objSettingsData.Content
			}
		)
	);
	oSelf.resize();
...
PerformanceView.renderSlider = function () {	
	...
	//console.log(oWeeksPerUnit);
	//console.log(iTotalWeeks);
...
PerformanceView.bindEvents4Slider = function () {
	...
	if (oSelf.slider_min_val == null) {
		...
		min_val = 1;
		max_val = (objCurrentWeekJsonData.Content.WeekNo == 1 ? 2 : objCurrentWeekJsonData.Content.WeekNo) || 2;
    }
	else {
	...
	/* if (
        ...
        for(var i = 1; i<=((total/5)+1) ; i++){
            var left = ( i > 1) ? ((i-1) * 8) + 2 : 0;
            var label = ( i> 1 ) ? ((i-1) * 5) : i;
            weekHtml+="<span class='weekSpan' style='left:"+left+"em'>Week "+label+"</span>";
        }
   	$("#" + PERFORMANCE.c_s_SLIDER).slider({
		...
		create : function(event,ui) {
			$('#slider-range.ui-slider-horizontal .ui-slider-handle').css({'margin-left': -5});
             $("#" + PERFORMANCE.c_s_SLIDER).parent().prepend(weekHtml);
		}
	});
...
* @return void;
*/
PerformanceView.plotGraph = function() { //alert("plot");
	var oSelf = this,
		sGraphType = (
			($('.menuclass li.active').length > 0)?
			$('.menuclass li.active:last > a').data('pgraph-type'):
	...
	oSelf.manageBenchMark = new (function () {
		...
		// method used for getting the marker colours based on benchMark
		this.calculateGLE = function (score, cWeekNumber) {
			var bMarkdata = oSelf.model.performanceInfoData.lexileGLEData,
				iScoreGLE = "",
				oSelectedWeekValue ="";
				//cWeek = cWeekNumber;
				var bMarkdata2 = PerformanceView.model.performanceInfoData.WeekLexileGLEMap[objPerformanceInfoJsonData.gradeId].gleInfo;
				...
				/* var bMarkdata2 = PerformanceView.model.performanceInfoData.lexileGLEData2[objPerformanceInfoJsonData.gradeId].gleInfo['level'+readingscore];
				$.each(bMarkdata2, function (id, val) {
					var maxMinweek = id.split("-");
					if(parseFloat(maxMinweek[0]) <= parseFloat(cWeekNumber) && typeof maxMinweek[1] == "undefined"){
						oSelectedWeekValue = val;
					}else {
						if (parseFloat(cWeekNumber) >= parseFloat(maxMinweek[0]) && parseFloat(cWeekNumber) <= parseFloat(maxMinweek[1])){
							oSelectedWeekValue = val;
						}
	...
	if(
		...
		sGraphType == PERFORMANCE.c_s_LBL_LIBRARY_RESPONSE_PROMPTS 
	) {
			...
			for (iJdx = 0; iJdx < aGraphPlottingSeries.length; iJdx++) {
				if (aGraphPlottingSeries[iJdx].name == sParameter) {
					if (aGraphPlottingSeries[iJdx].data[oRecord['UnitNumber'] + '-' + oRecord['WeekNumber']]) {
						...
						aGraphPlottingSeries[iJdx].data[oRecord['UnitNumber'] + '-' + oRecord['WeekNumber']]['lexileLevel'] += oSelf.manageBenchMark.calculateGLE(parseInt(oRecord['lexileLevel']),oRecord['WeekNumber']);
						aGraphPlottingSeries[iJdx].data[oRecord['UnitNumber'] + '-' + oRecord['WeekNumber']]['entries']++;
						break;
					}
					// aGraphPlottingSeries.length - 1
					aGraphPlottingSeries[iJdx].data[oRecord['UnitNumber'] + '-' + oRecord['WeekNumber']] = {
						'lexileLevel':	oSelf.manageBenchMark.calculateGLE(parseInt(oRecord['lexileLevel']),oRecord['WeekNumber']),
						'entries':	1
					};
					break;
				}
	...
	// Start:  plotting benchmark line for current week
		if(oSelf.currentWeekNo != null){
			var dataObj = {};
			dataObj.name = "Week Benchmark";
			dataObj.lineColor = "#7E7E7E";
			dataObj.enableMouseTracking = false;
			dataObj.dashStyle = "ShortDash";
			dataObj.data = [];
			currentWeek = oSelf.currentWeekNo*5;
			dataObj.data[0] = {"y":oSelf.yAxisMin,"x":(currentWeek),"marker":{"enabled":false}};
			dataObj.data[1] = {"y":oSelf.yAxisMax,"x": (currentWeek),"marker":{"enabled":false}};
			aGraphPlottingSeries.push(dataObj);
		}
	// End:  plotting benchmark line for current week
	var options = {
...
PerformanceView.effectOnSliderChange = function () {		 
	...
	//oSelf.slider_max_val = ($("#" + PERFORMANCE.c_s_SLIDER).slider("values")[1] == 3) ?  2 : $("#" + PERFORMANCE.c_s_SLIDER).slider("values")[1]; // -1 for slider manipulation
	...
	unit_selection_txt = oUtility.printf(
		PERFORMANCE.c_s_TXT_WEEKS + ' %d - %d',
		oSelf.fromWeekNo,
		(oSelf.toWeekNo == null) ? $GLOBALS.get('UnitWeekDetails').getTotalWeeks() : oSelf.toWeekNo 
	);
	...
	if($.isEmptyObject( aMasterData ) == false){
		...
		$.each(groupDetailView, function (subType, studentData) {
			...
			$.each(sortedStudentData, function (idx, valx) {
				...
				tempObj.ItemSubject 			= valx.ItemSubject;
				...
				// modification for ilit20
					if (
						tempObj.ItemSubType == "phonictextbasedslide" || 
						tempObj.ItemSubType == "extendedphonic" || 
						tempObj.ItemSubType == "frs"  
					) {						
						if (tempObj.ItemSubject == "pktof") {
							tempObj.ItemMaxScore = 100;
						}
					}
				// modification for ilit20
...
PerformanceView.renderWarning = function () {
	...
	$('#' + PERFORMANCE.c_s_RIGHT_PANEL_SUB_HEADER)
		.empty()
		.html(PERFORMANCE.c_s_WARNING_HEADER_TXT_DISPLAY ); //  objMinUnitNumber.UnitNumber
			...
			for(var key in objSubType) {				
				if(objSubType[key] == 'studyplan') {	
					...
					for(var cnt = 0; cnt < arrItems[0].length; cnt++) {	
						//	Search Through Student List						
						objData	=	_.filter(arrCompletedStudentList, function(obj) { 
										if( obj.IID == arrItems[0][cnt]) { return obj; }
									});
						if(objData.length != 0) {
							// arrFilteredStudent[objSubType[key]+"_"+objSubFilterData.ItemSubject][count++]	=	objData[0];
							for(var iObjCnt = 0; iObjCnt < objData.length;  iObjCnt++) {								
                    ...
                    case "Vocabulary":
                        ...
                        for(var key = 0; key < objData.length; key++) {		
                            if(typeof objData[key] != "undefined") {
                                var studentArr = _.where(objStudentListJsonData.Content, {UserID: objData[key].SID});	
                                if(sItemType == "studyplan") {
                                    calculatedPercentage	+=	(parseInt(objData[key].FS) / (parseInt(objData[key].IMS)) * 100);
                                    objModel["parentType"]	 =	"Vocabulary";
                                    objModel["type"]		 =	sItemType;
                                    objModel["ItemID"]		 =	objData[key].IID;
                                    objModel["StudentId"]	 =	objData[key].SID;
                        ...
                        for(var key = 0; key < objData.length; key++) {
                            if(typeof objData[key] != "undefined") {
                                ...
                                if(sItemType == "paragraph" ) {
                                    calculatedPercentage	+=	((parseInt(objData[key].FS) / 4) * 100);
                                ...
                                if(sItemType == "essay") {
                                    calculatedPercentage	+=	((parseInt(objData[key].FS) / 6) * 100);
                                    objModel["parentType"]	 =	"Writing";
                                    objModel["type"]		 =	sItemType;
                                    objModel["ItemID"]		 =	objData[key].IID;
                                    objModel["StudentId"]	 =	objData[key].SID;
                        ...
                        for(var key = 0; key < objData.length; key++) {	
                            if(typeof objData[key] != "undefined") {                            
                                ...
                                if(sItemType == "unitbenchmark") {
                                    calculatedPercentage	+=	((parseInt(objData[key].FS) / objData[key].IMS) * 100);
